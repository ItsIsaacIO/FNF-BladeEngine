...
class PlayState extends MusicBeatState
{
	...
	public var needsToDodge:Bool = false;
	public var opponentAttacking:Bool = false;
	public var doneDodging = true;

	public var attackNotifier:FlxSound;
	...
	public var camZoomingMult:Float = 1.2;
	...
	override public function create()
	{
		...
		healthLoss = ClientPrefs.getGameplaySetting('healthloss', 4);
		...
		bg.antialiasing = settings.stageAntialiasing;

		add(bg);

		defaultCamZoom = settings.stageZoom;

		BF_X = settings.bfPos[0];
		BF_Y = settings.bfPos[1];
		GF_X = settings.gfPos[0];
		GF_Y = settings.gfPos[1];
		DAD_X = settings.dadPos[0];
		DAD_Y = settings.dadPos[1];
		...
		switch (curStage) // For hardcoding other bg elements
		{
			case 'stage':
		...
		precacheList.set('notifyOfAttack', 'sound');
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				if (tmr.loopsLeft % boyfriend.danceEveryNumBeats == 0 && boyfriend.animation.curAnim != null && !boyfriend.animation.curAnim.name.startsWith('sing') && !boyfriend.stunned && doneDodging)
				{
					boyfriend.dance();
				}
				if (tmr.loopsLeft % dad.danceEveryNumBeats == 0 && dad.animation.curAnim != null && !dad.animation.curAnim.name.startsWith('sing') && !dad.stunned && !opponentAttacking)
				{
					dad.dance();
				}
				...
				switch (swagCounter)
				{
					case 0:
						...
						editStaticArrows(0, 1, Conductor.crochet / 1000, [0]);
						editStaticArrows(1, 1, Conductor.crochet / 1000, [0]);
					case 1:
						...
						editStaticArrows(0, 1, Conductor.crochet / 1000, [1]);
						editStaticArrows(1, 1, Conductor.crochet / 1000, [1]);
					case 2:
						...
						editStaticArrows(0, 1, Conductor.crochet / 1000, [2]);
						editStaticArrows(1, 1, Conductor.crochet / 1000, [2]);
					case 3:
						...
						editStaticArrows(0, 1, Conductor.crochet / 1000, [3]);
						editStaticArrows(1, 1, Conductor.crochet / 1000, [3]);
					case 4:
				}
	...
	public function countdownAfterStart():Void
		{
			var swagCounter:Int = 0;
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				if (gottaHitNote)
					swagNote = new Note(daStrumTime, daNoteData, oldNote, false, false, boyfriend.noteSkin, true);
				else
					swagNote = new Note(daStrumTime, daNoteData, oldNote, false, false, dad.noteSkin, false);
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus+1)
					{
						...
						if (gottaHitNote)
							sustainNote = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(songSpeed, 2)), daNoteData, oldNote, true, false, boyfriend.noteSkin, true);
						else
							sustainNote = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(songSpeed, 2)), daNoteData, oldNote, true, false, dad.noteSkin, false);
	...
	private function generateStaticArrows(player:Int, ?alpha:Float = 1):Void
	{
		for (i in 0...4)
		{
			// FlxG.log.add(i);
			...
			{
				babyArrow.alpha = 0;
				FlxTween.tween(babyArrow, {alpha: targetAlpha}, Conductor.crochet / 1000, {ease: FlxEase.circOut, startDelay: 0});
			}
			else
			{
	...
	public function editStaticArrows(player:Int, newAlpha:Float, alphaTweenTime:Float, notesToEdit:Array<Int>):Void
		{
			if (player == 1) {
				playerStrums.forEachAlive(function(noteToEdit:StrumNote){
					for (noteNum in notesToEdit) {
						if (noteToEdit.noteData == noteNum) {
							FlxTween.tween(noteToEdit, {alpha: newAlpha}, alphaTweenTime, {ease: FlxEase.circOut, startDelay: 0});
						}
					}
				});
			} else {
				if (dad.curCharacter != 'restricted-mouse') {
					opponentStrums.forEachAlive(function(noteToEdit:StrumNote){
						for (noteNum in notesToEdit) {
							if (noteToEdit.noteData == noteNum) {
								FlxTween.tween(noteToEdit, {alpha: newAlpha}, alphaTweenTime, {ease: FlxEase.circOut, startDelay: 0});
							}
	...
	{
		if (FlxG.keys.pressed.SPACE && needsToDodge)
			needsToDodge = false;
		...
		if (ratingFC == "" || ratingFC == null)
			ratingFC = "N/A";
			
		scoreTxt.text = 'Score: ' + songScore + ' - Misses: ' + songMisses + ' - Rating: ' + ratingFC;
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Opponent Attack':
				opponentAttacking = true; // This is for other things to detect if mouseys attacking, so dodging doesnt cancel out the variable
				needsToDodge = true;
				doneDodging = false;

				dad.playAnim('pre-attack', true);

				FlxG.sound.play(Paths.sound('notifyOfAttack', 'preload'), 1, false);
				
				var attackTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer) {
					var attackTimer2 = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer) {
						dad.playAnim('attack', true);

						if (needsToDodge) {
							health = 0;
						} else {
							boyfriend.animation.play('dodge', false);
						}

						needsToDodge = false;

						var doneDodgeTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer) {
							doneDodging = true;
			...
			case 'Fade Char In': 
				...
				switch(value1.toLowerCase().trim()) {
					...
					case 'gf' | 'girlfriend':
						char = gf;
					case 'dad' | 'opponent' | 'mousey':
						char = dad;	
				}

				if (char == dad) {
					FlxTween.tween(dad, {alpha: 1}, time, {ease: FlxEase.linear, startDelay: 0});
				} else if (char == boyfriend) {
					FlxTween.tween(boyfriend, {alpha: 0}, time, {ease: FlxEase.linear, startDelay: 0});
				}
			...
			case 'Fade Char Out': 
				...
				if (char == dad)
					FlxTween.tween(dad, {alpha: 0}, time, {ease: FlxEase.linear, startDelay: 0});
				else if (char == boyfriend)
					FlxTween.tween(boyfriend, {alpha: 0}, time, {ease: FlxEase.linear, startDelay: 0});
			...
			case 'Tween Cam Zoom':
				if(ClientPrefs.camZooms) {
					...
					var time:Float = Std.parseFloat(value2);
					if(Math.isNaN(camZoom)) camZoom = 1;
					if(Math.isNaN(time)) time = 5;

					FlxTween.tween(FlxG.camera, {zoom: camZoom}, time, {ease: FlxEase.linear, onComplete: function(callback:Dynamic) {
						defaultCamZoom = camZoom;
					}});
				}

			case 'Bop Camera':
				var oldZoom:Float = FlxG.camera.zoom;

				var zoomIn:Float = Std.parseFloat(value1);
				if(Math.isNaN(zoomIn)) zoomIn = 1.2;

				FlxTween.tween(FlxG.camera, {zoom: zoomIn}, Conductor.crochet / 1000 / 8, {ease: FlxEase.linear, 
					onComplete: function(twn:FlxTween) {
						FlxTween.tween(FlxG.camera, {zoom: oldZoom}, Conductor.crochet / 1000 / 8, {ease: FlxEase.linear, startDelay: 0});
					}
				});
			...
			case 'Change Scroll Speed':
				...
				{
					songSpeedTween = FlxTween.tween(this, {songSpeed: val1}, val2, {ease: FlxEase.linear, onComplete:
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achivementList = Achievements.achievementsStuff;

			for (i in 0...achivementList.length) {
				var curAchivement = achivementList[i][3];
				var achieve:String = checkForAchievement([curAchivement]);
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		FlxTween.tween(this, {health: health - (daNote.missHealth * healthLoss)}, Conductor.stepCrochet / 2000);
			...
			if(char != null)
			{
				...
				if(char.camFollowNotes) {
					if(!SONG.notes[Math.floor(curStep / 16)].mustHitSection) {
						...
						switch(animToPlay) {
							case 'singLEFT':
								FlxTween.tween(camFollow, {x: camFollow.x - 70}, 0.01, {ease: FlxEase.sineIn, startDelay: 0});
							case 'singDOWN':
								FlxTween.tween(camFollow, {y: camFollow.y + 50}, 0.01, {ease: FlxEase.sineIn, startDelay: 0});
							case 'singUP':
								FlxTween.tween(camFollow, {y: camFollow.y - 50}, 0.01, {ease: FlxEase.sineIn, startDelay: 0});
							case 'singRIGHT':
								FlxTween.tween(camFollow, {x: camFollow.x + 70}, 0.01, {ease: FlxEase.sineIn, startDelay: 0});
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			var daRating:Rating = Conductor.judgeNote(note, Math.abs(note.strumTime - Conductor.songPosition + ClientPrefs.ratingOffset));

			switch(daRating.name) {
				case 'shit':
					FlxTween.tween(this, {health: health - (note.missHealth * healthLoss)}, Conductor.stepCrochet / 2000);
				case 'bad':
					FlxTween.tween(this, {health: health - (note.missHealth * healthLoss / 2)}, Conductor.stepCrochet / 2000);
				default:
					FlxTween.tween(this, {health: health + (note.hitHealth * healthGain)}, Conductor.stepCrochet / 2000);
			}
	...
	// Random event trigger future update??!?!?!
	...
	override function beatHit()
	{
		...
		// Somethin for random events in a song
