...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Tween Cam Zoom', "Value 1: Zoom to transition into (Default: 1)\nValue 2: Time to transition (Default: 5)"],
		['Bop Camera', "Value 1: Zoom to bop to"],
		['Play Animation', "Plays an animation on a Character,\nonce the animation is completed,\nthe animation changes to Idle\n\nValue 1: Animation to play.\nValue 2: Character (Dad, BF, GF)"],	
		['Change Character', "Value 1: Character to change (Dad, BF, GF)\nValue 2: New character's name"],
		['Fade Char In', "Value 1: Character to change (Dad, BF, GF)\nValue 2: Time to tween"],
		['Fade Char Out', "Value 1: Character to change (Dad, BF, GF)\nValue 2: Time to tween"],
		['Countdown', "Counts down like at the start of the song."],
		['Change Scroll Speed', "Value 1: New Scroll Speed (2 is default)\nValue 2: Time to tween"],
		['Opponent Attack', "Makes The Current Opponent Attack"],
	...
	**/
	...
	function addSectionUI():Void
	{
		...
		/*var delLeftSecButton:FlxButton = new FlxButton(20 + copyButton.width, copyButton.y, "Clear Section (Left)", function() {
			for (note in _song.notes[curSection].sectionNotes) {
				if (note.playerNote && _song.notes[curSection].mustHitSection) {

					_song.notes[curSection].sectionNotes.remove(note);

				} else if (!note.playerNote && !_song.notes[curSection].mustHitSection) {
		...
		/*var delRightSecButton:FlxButton = new FlxButton(20 + pasteButton.width, pasteButton.y, "Clear Random"/Clear Section (Right)", function() {
			for (note in _song.notes[curSection].sectionNotes) {
				if (!note.playerNote && _song.notes[curSection].mustHitSection) {
		...
		//tab_group_section.add(delLeftSecButton);
		//tab_group_section.add(delRightSecButton);
