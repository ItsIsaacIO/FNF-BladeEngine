...
class CharacterEditorState extends MusicBeatState
{
	...
	function reloadBGs() {
		...
		if(onPixelBG) {
			...
			var bgSchool:BGSprite = new BGSprite('weeb/weebSchool', repositionShit, -playerYDifference + 6, 0.6, 1);
			bgLayer.add(bgSchool);
			bgSchool.antialiasing = false;

			var bgStreet:BGSprite = new BGSprite('weeb/weebStreet', repositionShit, -playerYDifference, 0.95, 1);
			...
			var bg:BGSprite = new BGSprite('stageback', -600 + OFFSET_X - playerXDifference, -300, 1, 1);
			bgLayer.add(bg);

			var stageFront:BGSprite = new BGSprite('stagefront', -650 + OFFSET_X - playerXDifference, 500, 1, 1);
	...
	function addCharacterUI() {
		...
		camFollowNotesBox = new FlxUICheckBox(flipXCheckBox.x + 110, flipXCheckBox.y + 80, null, null, "Camera Follows Note Direction", 80);
		camFollowNotesBox.checked = char.camFollowNotes;

		camFollowNotesBox.callback = function() {
			char.camFollowNotes = false;

			if(!camFollowNotesBox.checked) {
				char.camFollowNotes = true;
			}

			char.camFollowNotes = camFollowNotesBox.checked;
		};
		...
		var saveCharacterButton:FlxButton = new FlxButton(reloadImage.x, noAntialiasingCheckBox.y + 80, "Save Character", function() {
			saveCharacter();
		});
	...
	function genBoyOffsets():Void
	{
		...
		var offsetDisplay:FlxText = new FlxText(10, 20, 0, "Offsets:", 15);
		offsetDisplay.setFormat(null, 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		offsetDisplay.borderSize = 1;
		offsetDisplay.cameras = [camHUD];
		offsetDisplay.scrollFactor.set();
		add(offsetDisplay);

		for (anim => offsets in char.animOffsets)
		{
			var text:FlxText = new FlxText(10, 38 + (18 * daLoop), 0, anim + ": " + offsets, 15);
	...
	function saveCharacter() {
		var json = {
			...
			"cameraFollowNotes": char.camFollowNotes,
